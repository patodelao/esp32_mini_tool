/*
 * qmi8658.c — Implementación del driver QMI8658 sobre el I2C legacy del bsp.
 */
#include "qmi8658.h"
#include "bsp.h"
...
{
    return i2c_master_write_read_device(BSP_I2C_NUM, s_addr, &reg, 1,
...
static esp_err_t reg_write(uint8_t reg, uint8_t val)
{
    ...
    return i2c_master_write_to_device(BSP_I2C_NUM, s_addr, buf, sizeof(buf),
                                      pdMS_TO_TICKS(100));
}
...
static esp_err_t probe(uint8_t addr)
{
    s_addr = addr;
    uint8_t who = 0;
    esp_err_t err = reg_read(REG_WHO_AM_I, &who, 1);
