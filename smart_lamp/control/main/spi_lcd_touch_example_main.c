...
#include "esp_err.h"
#include "esp_log.h"
#include "lvgl.h" // <--- Asegúrate de tener esta dependencia
#include "sdkconfig.h"
...
// #include "driver/uart.h" // <--- Eliminado
#include <time.h>
#include <sys/time.h>
...
static const char *TAG = "EMISOR_SIMPLE"; // Tag cambiado

// Using SPI2 in the example
#define LCD_HOST  SPI2_HOST

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////// Configuración del LCD (Sin cambios) ///////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
static volatile bool wifi_connected = false; // <--- Flag de estado de Wi-Fi

static SemaphoreHandle_t lvgl_mux = NULL;
// static lv_obj_t *clock_label; // <--- Eliminado
// static lv_obj_t *wifi_btn; // <--- Eliminado

esp_lcd_touch_handle_t tp = NULL;
static SemaphoreHandle_t touch_mux = NULL;

// void connect_to_wifi(void); // <--- Eliminado
// extern void create_main_ui(); // <--- Eliminado


// --- Definiciones de funciones de hardware (sin cambios) ---
...
// --- Sección del Cliente HTTP (Sin cambios) ---
    ...
    switch(evt->event_id) {
        ...
        case HTTP_EVENT_ON_DATA:
            ...
            if (!esp_http_client_is_chunked_response(evt->client)) {
                 ESP_LOGI(TAG, "Respuesta del Receptor: %.*s", evt->data_len, (char*)evt->data);
            }
            break;
...
{
    char receptor_url[80];
    snprintf(receptor_url, sizeof(receptor_url), "http://%s/next-state", RECEPTOR_IP); 

    ESP_LOGI(TAG, "Enviando petición a: %s", receptor_url);
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "Petición HTTP exitosa, status = %d", esp_http_client_get_status_code(client));
    } else {
        ESP_LOGE(TAG, "Petición HTTP falló: %s", esp_err_to_name(err));
    }
    esp_http_client_cleanup(client);
}
// --- Fin de la sección del Cliente HTTP ---


// --- Callback para el botón de disparo remoto (Sin cambios) ---
static void remote_trigger_btn_cb(lv_event_t *e)
{
    ESP_LOGI(TAG, "Botón de disparo remoto presionado.");
    
    if (wifi_connected) {
        // Llama a la función que envía el HTTP GET
        trigger_remote_next_state();
    } else {
        ESP_LOGW(TAG, "No se puede enviar el comando, Wi-Fi desconectado.");
    }
}


// --- ¡NUEVA INTERFAZ! Solo un botón rojo ---
void create_simple_button_ui(void)
{
    ...
    lv_obj_set_size(remote_btn, 180, 180); // Botón grande
    lv_obj_align(remote_btn, LV_ALIGN_CENTER, 0, 0); 
    lv_obj_add_event_cb(remote_btn, remote_trigger_btn_cb, LV_EVENT_CLICKED, NULL);

    // --- Estilo del botón ---
    lv_obj_set_style_radius(remote_btn, LV_RADIUS_CIRCLE, LV_STATE_DEFAULT); // Redondo

    // --- Estado Normal (Rojo, levantado) ---
...
// --- Handler de Wi-Fi MODIFICADO para conexión automática ---
    ...
    int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        wifi_connected = false; // <-- Marca como desconectado
        esp_wifi_connect();
        ...
        wifi_connected = true; // <-- Marca como conectado
        
        // Inicia SNTP (sincronización de hora) una vez que tenemos IP
        initialize_sntp();
        setenv("TZ", "CLT4CLST,M9.1.6/23,M4.1.6/23", 1); // Zona horaria Chile
        tzset();
...
// void wifi_status_task(void *pvParameters) // <--- Eliminado
// { ... }
...
void wifi_init(void)
{
    ...
    wifi_event_group = xEventGroupCreate(); // Mover antes de registrar handlers

    ESP_ERROR_CHECK(esp_event_handler_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &wifi_event_handler, NULL));
    ESP_ERROR_CHECK(esp_event_handler_register(IP_EVENT, IP_EVENT_STA_GOT_IP, &wifi_event_handler, NULL));

    ESP_ERROR_CHECK(esp_wifi_start()); // <-- Esto inicia la conexión automáticamente
    ESP_LOGI(TAG, "wifi_init configurado para conexión automática.");
}

// void clock_task(void *pvParameters) // <--- Eliminado
// { ... }
// void wait_for_time_sync() // <--- Eliminado
// { ... }
...
void app_main(void){
    ...
    i2c_init();
    // uart_init(); // <--- Eliminado
    wifi_init(); // <-- Inicia el Wi-Fi y la conexión automática
    ...
    // --- 4. Inicializar Display (Backlight, SPI, Driver) ---
    ...
    // --- 7. Crear la Interfaz de Usuario ---
    ESP_LOGI(TAG, "Mostrar Interfaz Simple");
    if (example_lvgl_lock(-1)) {
        create_simple_button_ui(); // <-- Usamos la nueva función
        example_lvgl_unlock();
    }
}